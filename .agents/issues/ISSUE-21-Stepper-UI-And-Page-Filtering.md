...
### 1. Backend: Fungsi Ekstraksi Berdasarkan Halaman
...
- [ ] Ubah endpoint `POST /api/v1/soal/generate` di `routes/soal.py` agar bisa menerima *parameter* opsional baru: `page_ranges` (tipe `str`, contoh: `"1-3, 5"`).
...
- [ ] **Step 1: Sumber Materi.** Memilih antara modul dari *Library*, Upload Modul baru, atau Topik Manual.
- [ ] **Step 2: Filter Konteks (BARU).** 
  - Jika user memilih PDF, tampilkan *preview* dokumen menggunakan `<iframe src={file_url} className="w-full h-[500px]" />`. (Pastikan endpoint backend bisa menyajikan file statis dari folder `uploads/`).
